...
def solve(n_epochs, learning, n_x, n_t):
    """
    Trains the neural network with given input parameters and returns the network and the model objects
    """
    n_hidden = 100
...
def plot_nn(network, model, n_epochs, learning, n_x, n_t):
    """
    Plots the needed plots for an individual NN run
    """
    # x and t for plotting
    ...
    # plotting of the solution v exact result at four different timestamps
    ...
    fig.supxlabel("x")
    fig.tight_layout()
    fig.savefig("./figs/compare_nn_analytic_epoch%d_dx%d_dt%d.pdf" %(n_epochs, n_x, n_t))
    plt.close(fig)


    # plotting of the MSE as a function of epochs
...
def mse(a , b):
    """
    Computes the MSE given a and b
    """
    return np.mean((a-b)**2)

def test(network, model, n_x_test, n_t_test):
    """
    Tests the network with chosen resolution and plots the needed results.
    """
